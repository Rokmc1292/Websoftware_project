...
CREATE DATABASE IF NOT EXISTS nsns_db CHARACTER SET utf8mb4 -- utf8mb4 : 한글 + 이모지까지 저장 가능한 인코딩 (utf8보다 완전함)
...
-- ── 제약 조건 ──
PRIMARY KEY (id),
CONSTRAINT fk_wset_session FOREIGN KEY (session_id) REFERENCES workout_sessions (id) ON DELETE CASCADE,
...
CREATE TABLE diet_entries (
    ...
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
    CONSTRAINT fk_diet_entry_user FOREIGN KEY (user_id) REFERENCES users (id) ON DELETE CASCADE,
...
CREATE TABLE diet_items (
    ...
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
    CONSTRAINT fk_diet_item_entry FOREIGN KEY (entry_id) REFERENCES diet_entries (id) ON DELETE CASCADE,
...
-- -----------------------------------------------------------------------------
-- 6. fitbit_tokens — Fitbit API 토큰 저장 테이블
-- -----------------------------------------------------------------------------
-- 사용자의 Fitbit 데이터에 접근하기 위한 OAuth 2.0 토큰 정보 저장
-- 주기억: access_token, refresh_token
-- 갱신 주기: access_token은 1시간, refresh_token은 6개월

CREATE TABLE fitbit_tokens (
  id INT UNSIGNED NOT NULL AUTO_INCREMENT,
  user_id INT UNSIGNED NOT NULL,
  fitbit_user_id VARCHAR(100) DEFAULT NULL,
  ...
  updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  UNIQUE KEY uq_fitbit_tokens_user_id (user_id),
  CONSTRAINT fk_fitbit_token_user FOREIGN KEY (user_id) REFERENCES users (id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
