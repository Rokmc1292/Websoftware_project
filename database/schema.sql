...
--   4. diet_records       — 식단 기록 (음식명·칼로리·영양소)
--   5. sleep_records      — 수면 기록 (취침·기상·품질)
-- =============================================================================
...
CREATE DATABASE IF NOT EXISTS nsns_db
  ...
  COLLATE utf8mb4_unicode_ci;  -- ci(case insensitive) : 대소문자 구분 없이 정렬·비교
...
-- 1. users — 회원 정보 테이블
-- -----------------------------------------------------------------------------
-- 관계: workout_sessions, diet_records, sleep_records 테이블이 이 테이블을 참조함
-- (부모 테이블 — 자식 테이블들보다 먼저 생성해야 함)

DROP TABLE IF EXISTS users; -- 테이블이 이미 있으면 삭제 후 재생성 (개발 초기화 용도)
...
DROP TABLE IF EXISTS workout_sets;     -- workout_sets가 workout_sessions를 참조하므로 먼저 삭제
DROP TABLE IF EXISTS workout_sessions; -- 그 다음 workout_sessions 삭제
...
-- -----------------------------------------------------------------------------
-- 4. diet_records — 식단 기록 테이블
-- -----------------------------------------------------------------------------
-- 하루에 먹은 음식 하나하나를 기록
-- 수동 입력 또는 Claude Vision API로 음식 사진 → 자동 칼로리 추정 결과를 저장
...
CREATE TABLE diet_records (
  ...
  -- 식사 종류: breakfast(아침), lunch(점심), dinner(저녁), snack(간식)
  -- ENUM으로 제한 — 오타나 잘못된 값 입력 방지

  -- ── 음식 정보 ──
  food_name       VARCHAR(150)    NOT NULL,
  -- 음식 이름 (예: '닭가슴살 도시락', '그릭 요거트', '바나나')

  serving_size    VARCHAR(50)     DEFAULT NULL,
  -- 섭취량 (예: '1인분', '200g', '1개') — 다양한 표현이 있어 VARCHAR로 저장

  calories        SMALLINT UNSIGNED DEFAULT NULL,
  -- 칼로리 (kcal) — SMALLINT(0~65,535): 한 끼 65,535kcal를 넘을 일 없음

  -- ── 3대 영양소 (단위: g) ──
  carbs_g         DECIMAL(6, 2)   DEFAULT NULL,
  -- 탄수화물 (Carbohydrates) — 소수점 2자리 (예: 45.50 g)
  ...
  -- 사용자가 업로드한 음식 사진의 저장 경로 또는 URL
  -- NULL이면 수동 입력, 값이 있으면 AI Vision으로 분석된 기록

  is_ai_estimated TINYINT(1)      NOT NULL DEFAULT 0,
  -- 0 = 사용자가 직접 입력, 1 = AI가 추정한 값
  ...
  -- 특정 사용자의 특정 날짜 식단을 빠르게 조회하기 위한 복합 인덱스
  INDEX idx_diet_user_date (user_id, record_date),

  -- 날짜별 식사 종류로도 자주 조회하므로 추가 인덱스
...
--   │
--   ├──< diet_records (多)       : 한 사용자가 여러 식단 기록 보유
--   │
