...
class WorkoutSession(db.Model):
    ...
    duration_min = db.Column(SMALLINT(unsigned=True), nullable=True)
...
class WorkoutSet(db.Model):
    ...
    set_number = db.Column(SMALLINT(unsigned=True), nullable=False)
    ...
    reps = db.Column(SMALLINT(unsigned=True), nullable=True)

    # duration_sec : 시간 기반 운동의 지속 시간 (초 단위)
    # 예: 플랭크 60초 → duration_sec=60
    # 중량·횟수 기반 운동과 시간 기반 운동 중 하나를 선택해 사용
    duration_sec = db.Column(SMALLINT(unsigned=True), nullable=True)

    # rest_sec : 이 세트 후 휴식 시간 (초 단위, 선택 입력)
    rest_sec = db.Column(SMALLINT(unsigned=True), nullable=True)
